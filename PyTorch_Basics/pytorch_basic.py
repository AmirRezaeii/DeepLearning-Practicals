...
def multiples_of_ten(start, stop):
  ...
  if first_multiple <= last_multiple:
    x = torch.arange(first_multiple, last_multiple + 1, 10, dtype=torch.float64)
  else:
...
def slice_indexing_practice(x):
  ...
  assert x.shape[1] >= 5
  last_row = x[-1, :]
  third_col = x[:, 2:3]  # Using 2:3 keeps it 2D, not x[:, 2] which would be 1D
  first_two_rows_three_cols = x[0:2, 0:3]  # or x[:2, :3]
  even_rows_odd_cols = x[0::2, 1::2]  # or x[::2, 1::2]
  out = (
    last_row,
    third_col,
...
def slice_assignment_practice(x):
  ...
  Returns: x
  """
  x[0, 0:6] = torch.tensor([0, 1, 2, 2, 2, 2])
  x[1, 0:6] = torch.tensor([0, 1, 2, 2, 2, 2])
  x[2, 0:6] = torch.tensor([3, 4, 3, 4, 5, 5])
  x[3, 0:6] = torch.tensor([3, 4, 3, 4, 5, 5])
  return x
...
def make_one_hot(x):
  ...
  y[torch.arange(N), torch.tensor(x)] = 1
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    ...
    for i in range(x.shape[0]):
      batch_results.append(torch.mm(x[i], y[i]))
    z = torch.stack(batch_results, dim=0)
  else:
    z = torch.bmm(x, y)
  return z
